...
public class OnInitialize extends BasicEvent
{
   ...
   * Liefert ein Element der Suchstrategie mindestens einen Datensatz zurück, so
   * werden die anderen Elemente der Suchstrategie nicht mehr ausgewertet.
   * 
   * @param dsj
  ...
  private int searchDefaultSender(DatasourceJoiner dsj)
  {
    ...
    try
    {
      strat = wmConf.query("PersoenlicheAbsenderlisteInitialisierung").query("Suchstrategie").getLastChild();
    ...
    if (strat != null)
    {
      ...
      for (Iterator<ConfigThingy> iter = strat.iterator(); iter.hasNext();)
      {
        ...
        {
          Logger.error(L.m("Ungültiger Schlüssel in Suchstrategie: %1", element.stringRepresentation()));
        }
        if (found != 0)
          return found;
      }
    } else
    {
      ...
      return new ByOOoUserProfileFinder(dsj).find("Vorname", "${givenname}", "Nachname", "${sn}");
    }
  ...
  private static abstract class DataFinder
  {
     ...
     * evaluierten Wert von value enthält und überträgt die gefundenen Werte in
     * die PAL.
     * 
     * @param dbSpalte
    ...
    {
      Logger.debug2(this.getClass().getSimpleName() + ".tryToFind(" + dbSpalte + " '" + value + "')");
      try
      {
     ...
     * @return die Anzahl der gefundenen Datensätze
     */
    protected int find(String dbSpalte1, String value1, String dbSpalte2, String value2)
    {
      Logger.debug2(this.getClass().getSimpleName() + ".tryToFind(" + dbSpalte1 + " '" + value1 + "' " + dbSpalte2
          + " '" + value2 + "')");
      try
      {
   ...
   * getValueForKey im Benutzerprofil der OOo Registry nachschaut (das selbe wie
   * Extras->Optionen->OpenOffice.org->Benutzerdaten).
   * 
   * @author christoph.lutz
   */
  private static class ByOOoUserProfileFinder extends OnInitialize.DataFinder
  {
    ...
    protected String getValueForKey(String key)
    {
      try
      {
        UnoService confProvider = UnoService.createWithContext("com.sun.star.configuration.ConfigurationProvider",
            WollMuxSingleton.getInstance().getXComponentContext());

        UnoService confView = confProvider.create("com.sun.star.configuration.ConfigurationAccess",
   ...
   * @author christoph.lutz
   */
  private static class ByJavaPropertyFinder extends OnInitialize.DataFinder
  {
    ...
    protected String getValueForKey(String key)
    {
      ...
      {
        Logger.error(L.m("Konnte den Wert der JavaProperty '%1' nicht bestimmen:", key), e);
      }
      return "";
    }
  }
