...
import de.muenchen.allg.itd51.wollmux.event.WollMuxEventHandler.WollMuxEvent;
...
public class BasicEvent implements WollMuxEvent
{
  ...
  private boolean[] lock = new boolean[]
  { true };

  /**
   * Diese Method ist für die Ausführung des Events zuständig. Nach der
   * Bearbeitung entscheidet der Rückgabewert ob unmittelbar die Bearbeitung des
   * nächsten Events gestartet werden soll oder ob das GUI blockiert werden soll
   * bis das nächste actionPerformed-Event beim EventProcessor eintrifft.
   */
  @Override
  public void process()
  {
    ...
    {
      // hier wäre ein showNoConfigInfo möglich - ist aber nicht eindeutig auf
      // no config zurückzuführen
      errorMessage(e);
    }
  ...
  protected void errorMessage(Throwable t)
  {
     ...
     * 
     * 2. dies typischerweise passiert, wenn der Benutzer das Dokument
     * geschlossen hat, bevor der WollMux fertig war. In diesem Fall will er
     * nicht mit einer Meldung belästigt werden.
     */
  ...
  /**
   * Jede abgeleitete Event-Klasse sollte die Methode doit redefinieren, in der
   * die eigentlich event-Bearbeitung erfolgt. Die Methode doit muss alle
   * auftretenden Exceptions selbst behandeln, Fehler die jedoch
   * benutzersichtbar in einem Dialog angezeigt werden sollen, können über eine
   * WollMuxFehlerException nach oben weitergereicht werden.
   */
   ...
   * versucht die Absturzwahrscheinlichkeit von OOo/WollMux zu senken in dem es
   * den GarbageCollector der JavaVM triggert freien Speicher freizugeben. Durch
   * derartige Aufräumaktionen insbesondere nach der Bearbeitung von Events, die
   * viel mit Dokumenten/Cursorn/Uno-Objekten interagieren, wird die Stabilität
   * des WollMux spürbar gesteigert.
   * 
  ...
  /**
   * Setzt den Enable-Status aller OOo-Fenster, die der desktop aktuell liefert
   * auf enabled. Über den Status kann gesteuert werden, ob das Fenster
   * Benutzerinteraktionen wie z.B. Mausklicks auf Menüpunkte oder Tastendrücke
   * verarbeitet. Die Verarbeitung findet nicht statt, wenn enabled==false
   * gesetzt ist, ansonsten schon.
   * 
   * @param enabled
   */
   ...
   * Aufruf der waitForUnlock()-Methode. Der nicht modale Dialog erzeugt bei der
   ...
   * setUnlock() aufgehoben wird. So kann die quasi Modalität nicht modale
   * Dialoge zu realisiert werden. setLock() sollte stets vor dem Aufruf des
